...
public class MyCloth : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.Log($"Frame={Time.frameCount}, Energy={total_energy(vtx2xyz)}");
    ...
    Vector3[] gradient_spring(Vector3[] node2xyz, float length_ini, float stiffness)
    {
        ...
        float coefficient = stiffness / length * (length - length_ini);
        return new Vector3[2] { coefficient * (node2xyz[0] - node2xyz[1]), coefficient * (node2xyz[1] - node2xyz[0]) };
