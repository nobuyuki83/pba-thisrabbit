...
public class MyInflater : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length / 3; ++i_tri)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                bsm.AddBlockAt(node2vtx[i], num_vtx, new float3x3(grad_volume[i].x, 0, 0, grad_volume[i].y, 0, 0, grad_volume[i].z, 0, 0));
                bsm.AddBlockAt(num_vtx, node2vtx[i], new float3x3(grad_volume[i].x, grad_volume[i].y, grad_volume[i].z, 0, 0, 0, 0, 0, 0));
                rhs_vector[node2vtx[i]] += lambda * grad_volume[i];
            }
