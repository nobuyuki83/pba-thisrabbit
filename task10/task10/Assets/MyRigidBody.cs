...
public class MyRigidBody : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < 30; i++)
        {
            ...
            float3x3 inertia_tensor_inv = math.inverse(inertia_tensor);
            float3 L = math.mul(inertia_tensor, omega); // Angular momentum L = I * omega
            float3 d_omega_dt = math.mul(inertia_tensor_inv, -math.cross(omega, L));
            omega += timeStep * d_omega_dt;
    ...
    static float3x3 InertiaTensorForTriangleMesh(int[] tri2vtx, Vector3[] vtx2xyz)
    {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length / 3; i_tri++)
        {
            ...
            float3x3 skew = new float3x3(
                0, -pa.z, pa.y,
                pa.z, 0, -pa.x,
                -pa.y, pa.x, 0
            ) / 4.0f;
            res -= volume * math.mul(skew, skew);
            // end of edit
            // ------------------
        }
        return res;
